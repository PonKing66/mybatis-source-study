...
public class XMLMapperBuilder extends BaseBuilder {
  ...
  //解析
  public void parse() {
    //my 这里resource就是xml文件的全路径，com/xxx/xxx/xxx.xml
    //如果没有加载过再加载，防止重复加载
    ...
    //还有没解析完的东东这里接着解析？
    /**
     * my configureElement在解析ResultMap,因为ResultMap有extend属性，如果extend时，
     * 发现父类还没解析ResultMap,就会抛出IncompetentElementException异常，并且被加入到Configuration中
     * 的incompleteResultMaps集合
     * 这里就是去解析incompleteResultMaps内没有完成解析的resultMap
     */
    parsePendingResultMaps(); //解析未完成的ResultMap
    /**
     * 和上面类似，因为cache-ref是引用自其它映射文件缓存，所以也有可能存在它所引用的映射文件，还没完成解析时
     * 会被加入Configuration中的incompleteCacheRefs属性
     *
     */
    parsePendingChacheRefs(); //解析未完成的缓存
    /**
     * select标签上面可以使用useCache,需要确保适当确保当前的映射文件缓存所属的映射文件已经被加载，如果没有被加载，则
     * 当前标签会被加入Configuration中的incompleteStatements属性
     */
    parsePendingStatements(); //解析未完成Statements(Sql语句)
  }
